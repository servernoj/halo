...
namespace motor {
  std::vector<SegmentData> RampedMove::generateSegments(int32_t degrees, uint32_t factor) {
    ...
    int32_t midSegmentSize
      = static_cast<int32_t>(totalSteps) - static_cast<int32_t>(profile.size() * stepsPerSegment * 2);
    size_t profileCutPoint = midSegmentSize >= 0
                               ? profile.size()
                               : static_cast<size_t>(std::round(totalSteps / 2.0f / stepsPerSegment));
    ...
    for (size_t i = 0; i < profileCutPoint; ++i) {
      SegmentData seg;
      seg.steps = stepsPerSegment * dir;
      seg.period_us = static_cast<uint32_t>(std::round(START_PERIOD_US * profile[i] / factor));
      start.push_back(seg);
    }
