...
async function readU32LE(reg: number) {
  const b = Buffer.alloc(4)
  await bus.readI2cBlock(ADDR, reg, 4, b)
  return b.readUInt32LE(0)
}
async function readStatus(reg: number) {
  return bus.readByte(ADDR, reg)
}
async function pop(reg: number) {
...
async function condenseToLatest(statusReg: number, frontReg: number, backReg: number) {
  const JITTER_MS = 5
  while (true) {
...
// App-level state
let occupied = false
function onOccupied(t: number) {
  console.log(`[${new Date(t).toISOString()}] occupied`)
}
function onVacant(t: number) {
